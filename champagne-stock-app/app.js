...
/* ---------- Client Supabase (avec chargement de secours du CDN) ---------- */
const CONFIG_OK = typeof SUPABASE_URL === 'string' &&
...
function ensureSupabaseLib() {
  ...
  // Premier CDN indisponible (jsdelivr bloqué) : on essaie unpkg
  return new Promise(resolve => {
    ...
    setTimeout(() => resolve(!!window.supabase), 12000);
  });
}
...
async function initSupabase() {
  ...
  if (!window.supabase) {
    console.error('Librairie Supabase non chargée (CDN bloqué ?)');
    return false;
  }
  try {
...
function dbBrands() { return supabase.from('brands'); }
function dbEtats()  { return supabase.from('etats'); }
function dbProducts() { return supabase.from('products'); }
...
function setupRealtime() {
  ...
  try {
    realtimeChannel = supabase.channel('stock-changes');
...
function frAuthError(msg) {
  const m = String(msg || 'Erreur inconnue');
  if (m.includes('Invalid login') || m.includes('invalid_credentials')) return 'Email ou mot de passe incorrect.';
  if (m.includes('already registered')) return 'Cet email est déjà inscrit — connectez-vous.';
  if (m.includes('at least 6 characters')) return 'Le mot de passe doit contenir au moins 6 caractères.';
  if (m.includes('Email not confirmed')) return 'Veuillez confirmer votre email (lien reçu par email).';
  if (m.includes('Too many requests') || m.includes('rate limit')) return 'Trop de tentatives — patientez un instant.';
  if (m.includes('Failed to fetch') || m.includes('fetch')) return 'Pas de connexion internet. Vérifiez votre réseau.';
  return m;
}
...
function setAuthUi(session) {
  ...
  if (logged) {
    ...
    if (!CONFIG_OK) $('#authConfigHint').classList.remove('hidden');
  }
}
...
async function runDiag() {
  ...
  try {
    const res = await fetch(base + '/rest/v1/', { headers: { apikey: SUPABASE_ANON_KEY } });
    if (res.ok) {
      out.textContent = 'Serveur OK : URL et cl\u00e9 valides. Si l\'inscription reste bloqu\u00e9e, v\u00e9rifiez que schema.sql a \u00e9t\u00e9 ex\u00e9cut\u00e9 (SQL Editor).';
    } else if (res.status === 401) {
      out.textContent = 'Serveur r\u00e9pond (401) : l\'URL est bonne mais la cl\u00e9 anon semble incorrecte.';
    } else if (res.status === 404) {
      out.textContent = 'URL projet incorrecte (404) : v\u00e9rifiez le lien dans supabase-config.js.';
    } else {
      out.textContent = 'Serveur r\u00e9pond (code ' + res.status + ') : config probablement OK.';
    }
  } catch (e) {
    out.textContent = 'Connexion impossible : v\u00e9rifiez internet, puis l\'URL du projet.';
  }
}
...
function setupAuth() {
  ...
  const configMsg = '\u26A0 Supabase non configuré : ouvrez le fichier supabase-config.js et collez votre URL et votre clé anon (voir supabase.com > Settings > API). Puis rechargez la page.';

  // Badge de version (pour vérifier que l'app est bien à jour)
  $('#appVersion').textContent = 'version 6.0 — rechargée à ' +
    new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });

  // Diagnostic de connexion
  $('#diagRow').classList.toggle('hidden', !CONFIG_OK);
  $('#btnDiag').addEventListener('click', runDiag);

  if (!supabase) {
    // Tout clic doit donner un message explicite, jamais « rien »
    $('#authConfigHint').classList.remove('hidden');
    $('#authConfigHint').innerHTML = '\u26A0 <b>Inscription/Connexion indisponibles.</b><br>' +
      (CONFIG_OK ? 'La bibliothèque Supabase n\'a pas pu être chargée : vérifiez votre connexion internet et rechargez.' :
                  'Collez l\'URL et la clé anon de votre projet dans le fichier <b>supabase-config.js</b>, puis rechargez la page (F5).');
    $('#loginForm').addEventListener('submit', e => { e.preventDefault(); $('#authError').textContent = configMsg; });
    $('#signupForm').addEventListener('submit', e => { e.preventDefault(); $('#signupError').textContent = configMsg; });
    return;
  }
  ...
  $('#loginForm').addEventListener('submit', async e => {
    ...
    if (error) errEl.textContent = frAuthError(error.message);
  });
  ...
  $('#signupForm').addEventListener('submit', async e => {
    ...
    if (!email || !password) { errEl.textContent = 'Renseignez email et mot de passe.'; return; }
    if (password.length < 6) { errEl.textContent = 'Le mot de passe doit contenir au moins 6 caractères.'; return; }
    if (password !== password2) { errEl.textContent = 'Les mots de passe ne correspondent pas.'; return; }
    ...
    if (error) {
      errEl.textContent = frAuthError(error.message);
      return;
    }
    if (data && data.user && !data.session) {
      errEl.textContent = 'Compte créé ! Confirmez votre adresse email (lien reçu par email), puis connectez-vous.';
      document.querySelector('[data-form="login"]').click();
      window.confirm('Vérifiez votre boîte email pour confirmer votre inscription, puis connectez-vous.');
    }
  });
  ...
  $('#btnForgot').addEventListener('click', async () => {
    const email = $('#loginEmail').value.trim();
    if (!email) { $('#authError').textContent = 'Saisissez d\'abord votre email ci-dessus.'; return; }
    const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: location.href });
    $('#authError').textContent = error
      ? frAuthError(error.message)
  ...
  // Session en cours (y compris refresh / retour de l'onglet)
  supabase.auth.getSession().then(({ data }) => setAuthUi(data.session));
  supabase.auth.onAuthStateChange((event, session) => {
    if (event === 'SIGNED_OUT') setAuthUi(null);
    else if (session) setAuthUi(session);
  });
}
...
function setupEvents() {
  // Onglets Connexion / Inscription — en délégation pour être toujours fonctionnels
  // (indépendamment de l'état de Supabase / de la vitesse du CDN)
  document.addEventListener('click', e => {
    const tab = e.target.closest('.auth-tab');
    if (tab) switchAuthTab(tab.dataset.form);
  });
...
function init() {
  ...
  initSupabase().then(ok => {
    if (!ok) $('#authConfigHint').classList.remove('hidden');
    setupAuth();
  });
}
